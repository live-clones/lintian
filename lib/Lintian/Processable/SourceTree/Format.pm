...
package Lintian::Processable::SourceTree::Format;
...
Lintian::Processable::SourceTree::Format - Lintian interface to source format

=head1 SYNOPSIS

    my $collect = Lintian::Processable::SourceTree::Format->new;

=head1 DESCRIPTION

Lintian::Processable::SourceTree::Format provides an interface to source format
information.
...
has source_format => (
    ...
    default => sub {
        ...
        my $item = $self->patched->resolve_path('debian/source/format');

        return '1.0' unless $item->is_open_ok and $item->is_valid_utf8;

        my $format = $item->decoded_utf8;
        $format =~ s/\s*$//;
