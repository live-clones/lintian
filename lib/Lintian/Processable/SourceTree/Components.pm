...
package Lintian::Processable::SourceTree::Components;
...
Lintian::Processable::SourceTree::Components - interface to orig tar components

=head1 SYNOPSIS

   use Moo;

   with 'Lintian::Processable::SourceTree::Components';

=head1 DESCRIPTION

Lintian::Processable::SourceTree::Components provides an interface to data for
...
available alongside the source tree.  The key is the filename, and the value is
...
has components => (
    ...
    default => sub {
        ...
        my @files = $self->path->parent->children();
        ...
        for my $path (@files) {
            ...
            if ($path->basename
              ...
              $path->copy($self->basedir);
              $components{$path->basename} = $1 // $EMPTY;
            }
        }

        die encode_utf8("No orig archive found")
          unless scalar keys %components;
