...
# Spawning readelf is expensive: the parent may have a large resident
# set by the time the index is built, which makes each fork slow.
# Passing several files per invocation cuts the number of spawns while
# keeping the memory needed for the combined output within reason.
...
sub add_elf {
    ...
    while (my @batch = splice(@with_objects, 0, $BATCH_SIZE)) {
        ...
        if (valid_utf8($combined_bytes)) {
            ...
            $errors
              .= "Output from 'readelf --all --wide' on batch starting with "
              . $batch[0]->name
              . ' is not valid UTF-8'
              . $NEWLINE;
        }
        ...
        while (defined(my $fixed = shift @per_files)) {
            ...
            # chunks split from multi-file output start with the newline
            # that followed the File: line; remove it so standalone files
            # parse like single-argument output (archive members keep it,
            # as they always did)
            $per_file =~ s/^\n//
            ...
            unless (defined $file) {
                $errors
                  .= "Unknown container from readelf ($container)"
                  . $NEWLINE;
                next;
            }
