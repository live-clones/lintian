...
annotate-lintian-hints - give detailed information about Lintian's error tags

=head1 SYNOPSIS

B<annotate-lintian-hints> [I<log-file>...]

=head1 DESCRIPTION

The B<annotate-lintian-hints> command parses the EWI output of the
...
through B<annotate-lintian-hints> or a log file produced by B<lintian> can be
processed with this command.

  (Note, though, that the B<lintian> command has a command line option
B<-i> to display the same results as B<annotate-lintian-hints>, so you
...
Unlike B<lintian>, B<annotate-lintian-hints> will I<not> load any code from
