...
if ($type eq 'source') {
    ...
    foreach my $file (@files) {
        ...
        if (-f $templates_file) {
            my @templates;
            eval {
                @templates = read_dpkg_control ($templates_file,  DCTRL_DEBCONF_TEMPLATE);
            };
            if ($@) {
                chomp $@;
                $@ =~ s/^internal error: //;
...
my $selfrelation = Lintian::Relation->and ($info->relation ('provides'),
                                           $selfrel);
my $alldependencies = Lintian::Relation->and ($info->relation ('strong'),
                                              $selfrelation);
...
for my $file (qw(config prerm postrm preinst postinst)) {
    ...
    if ( -f $path and not -l $path) {
        ...
        tag 'postrm-does-not-purge-debconf'
            unless $type eq 'udeb' or $selfrelation->implies ($ANY_DEBCONF);
    }
}
