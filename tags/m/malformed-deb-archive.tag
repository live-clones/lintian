...
Explanation: The binary package is not a correctly constructed archive. A binary
 ...
 (or <code>data.tar.zst</code> on Ubuntu) in exactly that order. The
 <code>debian-binary</code> member must start with a single line containing the
 version number, with a major revision of 2.
See-Also: deb(5)
