...
Explanation: This tag is currently only issued for libcblas.so. For an explanation,
 ...
 .
 The symbols in <tt>libcblas.so</tt>, which represent the <tt>CBLAS</tt> API,
 were merged into <tt>libblas.so</tt>. (Note the missing letter <tt>C</tt>.)
 Please use <tt>libblas.so</tt> instead.
 .
 The old library is still being shipped until all packages have modified
 their build systems, but it is not managed by <tt>update-alternatives</tt>
 ...
 implemented by other <tt>BLAS/CBLAS</tt> alternatives. Please override the
